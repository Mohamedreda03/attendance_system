...
const toolImplementations = {
  ...
  search_users: async ({ role, level, name, user }) => {
    try {
      ...
      // Doctors can only search students enrolled in their courses
      if (user.role === ROLES.DOCTOR) {
  ...
  update_attendance_status: async ({ attendanceId, status, reason, user }) => {
    try {
      ...
      // Doctor can only update records for their courses
  ...
  get_student_attendance: async ({ studentId, courseId, user }) => {
    try {
      ...
      // Doctors can only see students in their own courses
      if (user.role === ROLES.DOCTOR) {
        const myCourses = await Course.find({ doctor: user._id }).select("_id");
        ...
        if (courseId) {
          ...
          // If no specific course requested, only show records from doctor's courses
...
const chat = async (messages, user = { role: ROLES.ADMIN }) => {
  ...
  try {
      ...
      if (responseMessage.tool_calls) {
        for (const toolCall of responseMessage.tool_calls) {
          ...
          try {
            ...
            console.error(`[Groq SDK] Error parsing tool arguments for ${functionName}:`, e);
            functionArgs = {};
          }
          
          // Ensure functionArgs is an object before setting user
          ...
          console.log(`[Groq SDK] Executing: ${functionName} for user: ${user.email}`);
    ...
    throw new Error(`خطأ في الاتصال بـ Groq: ${error.message}`);
  }
};
